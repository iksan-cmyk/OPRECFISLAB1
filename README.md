...
| `link_interview` | text, nullable | link meeting (Zoom/dll), diisi admin, dibaca publik |
...
- `pendaftar_aslab` SELECT: `authenticated` (admin) bebas semua kolom; `anon` diizinkan tapi dibatasi column-level GRANT ke 5 kolom saja (`nama_lengkap`, `nrp`, `status`, `jadwal_interview`, `link_interview`) — dipakai oleh halaman cek status publik.
...
Kalau status `lolos_berkas` dan `jadwal_interview` sudah diisi, tanggal ditampilkan via `toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })`. Kalau belum diisi, tampilkan "Jadwal interview menyusul, pantau terus ya". Selain jadwal, kalau `link_interview` terisi, blok "🔗 Link Interview" juga dirender sebagai tautan `target="_blank" rel="noopener noreferrer"` (tampil independen, walau jadwal belum diisi).

## Admin panel

- Login via Supabase Auth (email + password); akun dibuat manual lewat Supabase Dashboard — **tidak ada halaman signup**.
- Per baris tabel pendaftar: tombol **Detail** (modal dokumen + signed URL, semua baris), dropdown **status** (semua baris), kontrol **jadwal interview + link interview** — input `datetime-local` + input `url` (link Zoom/dll) + tombol Simpan — khusus baris berstatus `lolos_berkas`, menggantikan tombol "Dokumen" yang sudah dihapus. Satu tombol Simpan menyimpan jadwal + link sekaligus.
- Kalau status diubah keluar dari `lolos_berkas`, `jadwal_interview` dan `link_interview` **tidak** di-clear (dibiarkan untuk histori).
...
- `origin` → `KimaK-ops/oprec_fislab1-admin_dashboard.git` (fork dengan admin panel — push ke sini)
- `iksan` → `iksan-cmyk/OPRECFISLAB1.git`